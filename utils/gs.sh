                    ...
                    for epochs in ${EPOCHS[@]} ; do 
                        for bs in ${BS[@]} ; do
                            for max_lr in ${MAX_LR[@]} ; do
                                python3 e.py --size=$size --size_after_reshape=$size_after_reshape --step_size_ratio=$step_size_ratio --loss_weights="$loss_weights" --not_empty_ratis=$not_empty_ratis --epochs=$epochs --bs=$bs --max_lr=$max_lr
                            done
                        done
                    done
                done
